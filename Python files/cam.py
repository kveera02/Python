...
GEAR_BTN_POS = []
PLAY_BTN_POS = []
WORKING_BTN_POS = [100 , 100]
SPINNER_BTN_POS = [150 , 200]
NEXT_BTN_POS = SCREEN_WIDTH - 80
VIEW_MOUSE = True
...
sizeMode        =  1      # Image size; default = Large
...
sizeData = [ # Camera parameters for different size settings
 ...
 [(2592, 1944), (SCREEN_WIDTH, SCREEN_HEIGHT), (0.0   , 0.0   , 1.0   , 1.0   )], # Large
 [(1920, 1080), (SCREEN_WIDTH, 180), (0.1296, 0.2222, 0.7408, 0.5556)], # Med
 [(1440, 1080), (SCREEN_WIDTH, SCREEN_HEIGHT), (0.2222, 0.2222, 0.5556, 0.5556)]] # Small

isoData = [ # Values for ISO settings [ISO value, indicator X position]
 [  0,  27], [100,  64], [200,  97], [SCREEN_WIDTH, 137],
...
btn1 = SCREEN_WIDTH/8
radioBtns = [ btn1 , btn1 + 100 , btn1 + 250 , SCREEN_HEIGHT/4]
buttons = [
  # Screen mode 0 is photo playback
  [Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1], SCREEN_WIDTH, 52), bg='done' , cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev' , cb=imageCallback, value=-1),
   Button((NEXT_BTN_POS,  0, 80, 52), bg='next' , cb=imageCallback, value= 1),
  ...
  [Button((  0,35,SCREEN_WIDTH, 33), bg='delete'),
  ...
  [Button((0,  0,SCREEN_WIDTH,SCREEN_HEIGHT), cb=doneCallback), # Full screen = button
   Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1],SCREEN_WIDTH, 52), bg='done'),       # Fake 'Done' button
   Button((0, 53,SCREEN_WIDTH, 80), bg='empty')],     # 'Empty' message

  # Screen mode 3 is viewfinder / snapshot
  [Button((  50,250,156, 52), bg='gear', cb=viewCallback, value=0),
   Button((210,250,156, 52), bg='play', cb=viewCallback, value=1),
   Button((  0,  0,SCREEN_WIDTH,SCREEN_HEIGHT)           , cb=viewCallback, value=2),
  ...
  [Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1],SCREEN_WIDTH, 52), bg='done', cb=doneCallback),
   ...
   Button((NEXT_BTN_POS,  0, 80, 52), bg='next', cb=settingCallback, value=1),
   ...
   Button((0, 10,SCREEN_WIDTH, 35), bg='storage')],
  ...
  [Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1], SCREEN_WIDTH, 52), bg='done', cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev', cb=settingCallback, value=-1),
   Button((NEXT_BTN_POS,  0, 80, 52), bg='next', cb=settingCallback, value= 1),
   Button((radioBtns[0], radioBtns[3],100,120), bg='radio3-1', fg='size-l',
    cb=sizeModeCallback, value=0),
   Button((radioBtns[1], radioBtns[3],100,120), bg='radio3-0', fg='size-m',
    cb=sizeModeCallback, value=1),
   Button((radioBtns[2], radioBtns[3],100,120), bg='radio3-0', fg='size-s',
    cb=sizeModeCallback, value=2),
   Button((  0, 10,SCREEN_WIDTH, 29), bg='size')],
  ...
  [Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1], SCREEN_WIDTH, 52), bg='done', cb=doneCallback),
   ...
   Button((NEXT_BTN_POS, 70, 80, 52), bg='next', cb=fxCallback     , value= 1),
   Button((  0, 67,SCREEN_WIDTH, 91), bg='fx-none'),
   Button((  0, 11,SCREEN_WIDTH, 29), bg='fx')],
  ...
  [Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1],SCREEN_WIDTH, 52), bg='done', cb=doneCallback),
   ...
   Button((SCREEN_WIDTH-80, 70, 80, 52), bg='next', cb=isoCallback    , value= 1),
   Button((  0, 79,SCREEN_WIDTH, 33), bg='iso-0'),
   Button((  9,134,302, 26), bg='iso-bar'),
   Button(( 17,157, 21, 19), bg='iso-arrow'),
   Button((  0, 10,SCREEN_WIDTH, 29), bg='iso')],

  # Screen mode 8 is quit confirmation
  [Button((DONE_BUTTON_POS[0], DONE_BUTTON_POS[1],SCREEN_WIDTH, 52), bg='done'   , cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev'   , cb=settingCallback, value=-1),
   Button((NEXT_BTN_POS,  0, 80, 52), bg='next'   , cb=settingCallback, value= 1),
   Button((200, 100, 100, 120), bg='quit-ok', cb=quitCallback),
   Button((  0, 10,SCREEN_WIDTH, 35), bg='quit')]
]
...
def takePicture():
	...
	if scaled:
	  if scaled.get_height() < SCREEN_HEIGHT: # Letterbox
	    screen.fill(0)
	  screen.blit(scaled,
...
pygame.init()
pygame.mouse.set_visible(VIEW_MOUSE)
...
while(True):
  ...
  if img is None or img.get_height() < SCREEN_HEIGHT: # Letterbox, clear background
    screen.fill(0)
  if img:
    screen.blit(img,
      ((SCREEN_WIDTH - img.get_width() ) / 2,
